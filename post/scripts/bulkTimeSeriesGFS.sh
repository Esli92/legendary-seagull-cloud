...
DIRECTORIO_FECHAS=../../WRFrun/PBS_scripts/GFSfechas
DATA_DIR=/home/esli/Documentos/WRF/experimentos/verificacion/input/20092015_0p${RESOLUTION}

#Empezamos quitando todo
rm -rf ../timeSeriesGFS/${MES}_0p${RESOLUTION}
cp -r ../timeSeriesGFS/template ../timeSeriesGFS/${MES}_0p${RESOLUTION}
rm *.txt 
...
do
	...
	if [ ! -d "../timeSeriesGFS/${MES_INIT}_0p${RESOLUTION}" ]
	then
	    mkdir ../timeSeriesGFS/${MES_INIT}_0p${RESOLUTION}
	fi

	InputDir=$DATA_DIR/gfs.0p${RESOLUTION}.${ANIO}${MES_INIT}${DIA_INIT}00.fall.ctl

	sed '1 s:'/home/esli/Documentos/WRF/experimentos/verificacion/post/input/[0-9][0-9]_0p[0-9][0-9]/wrfout_GRADS_[0-9][0-9]-[0-9][0-9]_[0-9][0-9]_[0-9][0-9][0-9][0-9]_d[0-9][0-9].ctl':'${InputDir}':g' getTimeSeriesGFS.gs.template > getTimeSeries.gs
	sed '1 s:'/home/esli/Documentos/WRF/experimentos/verificacion/post/input/[0-9][0-9]_0p[0-9]/wrfout_GRADS_[0-9][0-9]-[0-9][0-9]_[0-9][0-9]_[0-9][0-9][0-9][0-9]_d[0-9][0-9].ctl':'${InputDir}':g' getTimeSeriesGFS.gs.template > getTimeSeries.gs	
	...
	#Limpiar un poco
	mv *.txt ../timeSeriesGFS/${MES_INIT}_0p${RESOLUTION}/${STATION}/2015
