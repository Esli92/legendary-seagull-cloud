...
DIRECTORIO_SALIDAS=${DIRECTORIO_DATOS}/dataFiles/pares/48h
	...
	then
		...
		if [ "$DEBUG_MODE" = false ]; then
			...
			#Para cuando tenga todas las variables
				...
				if [ "$VARIABLE" = TM ]; then
					find . -name '*TM.txt' -exec python ../../../../../observaciones/scripts/mkObsFrcstPairs_48hrs.py ${STATION}_${VARIABLE}.txt {} \;
				elif [ "$VARIABLE" = WS ]; then
					find . -name '*WS.txt' -exec python ../../../../../observaciones/scripts/mkObsFrcstPairs_48hrs.py ${STATION}_${VARIABLE}.txt {} \;
					echo 'nothing to do here either!' 
				elif [ "$VARIABLE" = RN ]; then
					find . -name '*RA.txt' -exec python ../../../../../observaciones/scripts/mkObsFrcstPairs_48hrs.py ${STATION}_${VARIABLE}.txt {} \;
				fi
