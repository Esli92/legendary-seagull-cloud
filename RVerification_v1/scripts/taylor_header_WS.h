...
# RESOLUCION 0.5
#------- ESTACION ACO -----------------------------------------------------------------------------------------------------------------------------
DAT_5_ALL_ACO <- read.table("./l24h/0pRESOLUTION/seasonal/SEASON/AJU_SEASON_VARIABLE.txt",sep = ",", header = TRUE, fill = FALSE)
OBS_5_ALL_ACO <- DAT_5_ALL_ACO$OBSERVACION
FCT_5_ALL_ACO <- DAT_5_ALL_ACO$PRONOSTICO

jpeg(file = "./plots/taylor/TAYLOR_VARIABLE_SEASON_STATIONS.jpg", width=1800,height=1500,units="px",quality=90,pointsize=33)

taylor.diagram(OBS_5_ALL_ACO,FCT_5_ALL_ACO,add=FALSE,col="red",pch=19,pos.cor=FALSE,
               main=" ",show.gamma=TRUE,ngamma=4,
...
lpos<-2*(sd(OBS_5_ALL_ACO)/sd(OBS_5_ALL_ACO))
lposy <- lpos+2
